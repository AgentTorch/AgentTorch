def debug():
    ...
    package_root_directory = os.path.dirname(os.path.dirname(os.path.dirname(current_directory)))
...
if __name__ == "__main__":
    ...
    from agent_torch.dataloader import LinkPopulation
    from populations import NYC
    from models import covid
    
    nyc_population = LinkPopulation(NYC) # all data for config.simulation_metadata
    simulation = Executor(model = covid, pop_loader=nyc_population) # _update_config(), _get_runner()
