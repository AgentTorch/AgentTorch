...
class CalibNN(nn.Module):
    def __init__(self,
        ...
        self.param_out_layer = nn.Linear(in_features=out_layer_width // 2,
...
class LearnableParams(nn.Module):
    ...
    def __init__(self, num_params, device, scale_output='abm-covid'):
        ...
        self.min_values = torch.tensor(MIN_VAL_PARAMS[scale_output][:self.num_params],
                                       device=self.device)
        self.max_values = torch.tensor(MAX_VAL_PARAMS[scale_output][:self.num_params],
                                       device=self.device)
        self.sigmoid = nn.Sigmoid()
