...
from AgentTorch.helpers import discrete_sample, get_by_path, logical_and, logical_not
...
class UpdateQuarantineStatus(SubstepTransition):
    ...
    def _end_quarantine(self, t, is_quarantined, quarantine_start_date):
        ...
        is_quarantined = is_quarantined + agent_quarantine_ends*self.END_QUARANTINE_VAR
        quarantine_start_date = quarantine_start_date*(1 - agent_quarantine_ends) + (self.INFINITY_TIME)*agent_quarantine_ends

        return is_quarantined, quarantine_start_date

    def _start_quarantine(self, t, is_quarantined, quarantine_start_date, agent_quarantine_start_action):
        agents_quarantine_start = agent_quarantine_start_action.long()

        is_quarantined = is_quarantined + agents_quarantine_start*self.START_QUARANTINE_VAR
        quarantine_start_date = quarantine_start_date*logical_not(agents_quarantine_start) + agents_quarantine_start*t
#         quarantine_start_date = quarantine_start_date*(1 - agents_quarantine_start) + (agents_quarantine_start)*t

        return is_quarantined, quarantine_start_date

    def _break_quarantine(self, t, is_quarantined, quarantine_start_date, agent_quarantine_break_action):
        agents_quarantine_break = agent_quarantine_break_action.long()

        is_quarantined = is_quarantined + agents_quarantine_break*self.BREAK_QUARANTINE_VAR
        quarantine_start_date = quarantine_start_date*logical_not(agents_quarantine_break) + agents_quarantine_break * self.INFINITY_TIME
#         quarantine_start_date = quarantine_start_date*(1 - agents_quarantine_break) + (self.INFINITY_TIME)*agents_quarantine_break

        return is_quarantined, quarantine_start_date

    def update_quarantine_status(self, t, is_quarantined, quarantine_start_date, agent_quarantine_start_action, agent_quarantine_break_action):
        is_quarantined, quarantine_start_date = self._end_quarantine(t, is_quarantined, quarantine_start_date)
        is_quarantined, quarantine_start_date = self._start_quarantine(t, is_quarantined, quarantine_start_date, agent_quarantine_start_action)
        is_quarantined, quarantine_start_date = self._break_quarantine(t, is_quarantined, quarantine_start_date, agent_quarantine_break_action)
    ...
    def forward(self, state, action):
        ...
        new_is_quarantined, new_quarantine_start_date = self.update_quarantine_status(t, is_quarantined, quarantine_start_date, agent_quarantine_start_action, agent_quarantine_break_action)
