...
from AgentTorch.helpers import get_by_path, logical_and, logical_not, logical_or, discrete_sample
...
class UpdateTestStatus(SubstepTransition):
    def __init__(self, config, input_variables, output_variables, arguments):
        ...
        self.test_ineligible_days = self.config['simulation_metadata']['test_ineligible_days']
        self.test_result_delay_days = self.config['simulation_metadata']['test_result_delay_days']

        self.AWAITING_RESULT_VAR = 1
        self.GOT_RESULT_VAR = -1
        self.INFINITY_TIME = self.config['simulation_metadata']['INFINITY_TIME']

    def get_test_result(self, t, agents_awaiting_results, agent_result_date, current_stages, test_re_eligble_date, true_positive_prob, false_positive_prob):
        ...
        true_positive_mask = discrete_sample(sample_prob=true_positive_prob, size=(self.num_agents,), device=self.device).unsqueeze(1)
        true_positive_results = logical_and(true_positive_result_candidates, true_positive_mask)

        false_positive_mask = discrete_sample(sample_prob=false_positive_prob, size=(self.num_agents,), device=self.device).unsqueeze(1)
        ...
        test_re_eligble_date[agents_result_expected_today.bool()] = t + self.test_ineligible_days # not a differentiable op
        agent_result_date[agents_result_expected_today.bool()] = self.INFINITY_TIME

        return positive_results, agents_awaiting_results, test_re_eligble_date, agent_result_date
    
    def get_tested(self, t, agents_awaiting_results, agents_result_date, test_enrolled_agents):
...
#         test_eligible = logical_and(test_eligible, logical_not(agents_awaiting_results))
        ...
        agents_result_date[test_enrolled_agents.bool()] = t + self.test_result_delay_days # not a differentiable op
    ...
    def forward(self, state, action=None):
        ...
        agents_result_date = get_by_path(state, re.split('/', input_variables['test_result_date']))
        agents_awaiting_results = get_by_path(state, re.split('/', input_variables['awaiting_test_result']))
#         is_quarantine_eligible = get_by_path(state, re.split('/', input_variables['is_quarantine_eligible']))
        ...
        true_positive_prob = get_by_path(state, re.split("/", self.input_variables["test_true_positive_prob"]))
        false_positive_prob = get_by_path(state, re.split("/", self.input_variables["test_false_positive_prob"]))

        # step 1: agents receive test result and may test positive
        positive_results, agents_awaiting_results, test_re_eligble_date, agent_result_date = self.get_test_result(t, agents_awaiting_results, agents_result_date, current_stages, test_re_eligble_date, true_positive_prob, false_positive_prob)
...
#         is_quarantine_eligible = logical_or(is_quarantine_eligible, positive_results)
        ...
        test_ineligible = logical_or(test_ineligible_cooldown, agents_awaiting_results).long()
        test_enrolled_agents = logical_and(test_willing_agents, logical_not(test_ineligible))

        agents_awaiting_results, agents_result_date = self.get_tested(t, agents_awaiting_results, agents_result_date, test_enrolled_agents)

        return {self.output_variables[0]: agents_awaiting_results, self.output_variables[1]: agents_result_date, self.output_variables[2]: test_re_eligble_date}