...
class UpdateMacroRates(SubstepTransition):
    ...
    def __init__(self, config, input_variables, output_variables, arguments):
        ...
        self.initial_claims = torch.tensor(initial_claims_df['beneficiaries'].values) # load initial claims data for each county separately. Current is kings county
    ...
    def forward(self, state, action):
        ...
        total_labor_force = torch.sum(working_status) # total number of agents willing to work
        labor_force_participation_rate = total_labor_force / self.num_agents
        
        ## EQ 2
        current_total_unemployment_rate = unemployment_adaptation_coefficient_all[0]*torch.log(total_labor_force) + unemployment_adaptation_coefficient_all[1]*torch.log(current_month_initial_claims) + unemployment_adaptation_coefficient_all[2] #+ self.labor_force_participation_rate_weight*(labor_force_participation_rate)
