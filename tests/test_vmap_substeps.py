...
from agent_torch.models.predator_prey.vmap_substeps.vmap_move import FindNeighborsVmap, DecideMovementVmap, UpdatePositionsVmap
from agent_torch.models.predator_prey.vmap_substeps.vmap_eat import FindEatableGrassVmap, EatGrassVmap
from agent_torch.models.predator_prey.vmap_substeps.vmap_hunt import FindTargetsVmap, HuntPreyVmap
...
class TestVmapMove(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.state = {
            "environment": {
                "bounds": torch.tensor([3, 3])  # 3x3 grid
            },
            "network": {
                "agent_agent": {
                    "predator_prey": {
        ...
        input_vars = {"bounds": "environment/bounds", "adj_grid": "network/agent_agent/predator_prey/adjacency_matrix", "positions": "agents/predator/coordinates"}
        self.find_neighbors = FindNeighborsVmap(None, input_vars, ["possible_neighbors"], {"learnable": {}, "fixed": {}})
        
        input_vars = {"positions": "agents/predator/coordinates", "energy": "agents/predator/energy"}
        self.decide_movement = DecideMovementVmap(None, input_vars, ["next_positions"], {"learnable": {}, "fixed": {}})
        ...
        self.update_positions = UpdatePositionsVmap(None, input_vars, ["prey_pos", "prey_energy", "pred_pos", "pred_energy"], {"learnable": {}, "fixed": {}})
    ...
    def test_find_neighbors(self):
        ...
        self.assertTrue(torch.equal(neighbors0[0], torch.tensor([0, 1])) or 
                        torch.equal(neighbors0[0], torch.tensor([1, 0])))
    ...
    def test_decide_movement(self):
        ...
        observations = {
            "possible_neighbors": [
                ...
                torch.tensor([[0, 1], [1, 0], [1, 2], [2, 1]]),  # Neighbors for predator 1
    ...
    def test_update_positions(self):
        ...
        self.assertTrue(torch.equal(result["prey_pos"], torch.tensor([[0, 2], [1, 1], [2, 0]])))
        self.assertTrue(torch.equal(result["pred_pos"], torch.tensor([[0, 1], [1, 2], [2, 2]])))
        
        # Check that energy was reduced by stride_work
        self.assertTrue(torch.equal(result["prey_energy"], torch.tensor([[7.0], [5.0], [3.0]])))
        self.assertTrue(torch.equal(result["pred_energy"], torch.tensor([[9.0], [4.0], [0.0]])))
...
class TestVmapEat(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Initialize test instances
        input_vars = {"bounds": "environment/bounds", "positions": "agents/prey/coordinates", "grass_growth": "objects/grass/growth_stage"}
        self.find_eatable_grass = FindEatableGrassVmap(None, input_vars, ["eatable_grass_positions"], {"learnable": {}, "fixed": {}})
        ...
        self.eat_grass = EatGrassVmap(None, input_vars, ["energy", "grass_growth", "growth_countdown"], {"learnable": {}, "fixed": {}})
    ...
    def test_find_eatable_grass(self):
        ...
        for pos in eatable_positions:
            ...
            self.assertEqual(self.state["objects"]["grass"]["growth_stage"][node].item(), 1)
    ...
    def test_eat_grass(self):
        ...
        self.assertTrue(torch.equal(result["energy"], torch.tensor([[15.0], [10.0], [7.0]])))
        ...
        self.assertEqual(result["growth_countdown"][0].item(), 20)  # Reset to regrowth_time
...
class TestVmapHunt(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Initialize test instances
        input_vars = {"prey_pos": "agents/prey/coordinates", "pred_pos": "agents/predator/coordinates"}
        self.find_targets = FindTargetsVmap(None, input_vars, ["target_positions"], {"learnable": {}, "fixed": {}})
        ...
        self.hunt_prey = HuntPreyVmap(None, input_vars, ["prey_energy", "pred_energy"], {"learnable": {}, "fixed": {}})
    ...
    def test_find_targets(self):
        ...
        self.assertTrue(any(torch.equal(pos, torch.tensor([0, 0])) for pos in target_positions))
        self.assertTrue(any(torch.equal(pos, torch.tensor([1, 1])) for pos in target_positions))
    ...
    def test_hunt_prey(self):
        ...
        self.assertEqual(result["prey_energy"][0].item(), 0)  # Prey 0 at (0,0) was eaten
        self.assertEqual(result["prey_energy"][1].item(), 0)  # Prey 1 at (1,1) was eaten
        self.assertEqual(result["prey_energy"][2].item(), 4)  # Prey 2 at (0,1) was not eaten
...
class TestVmapGrow(unittest.TestCase):
    ...
    def setUp(self):
        ...
        input_vars = {"grass_growth": "objects/grass/growth_stage", "growth_countdown": "objects/grass/growth_countdown"}
        self.grow_grass = GrowGrassVmap(None, input_vars, ["grass_growth", "growth_countdown"], {"learnable": {}, "fixed": {}})
