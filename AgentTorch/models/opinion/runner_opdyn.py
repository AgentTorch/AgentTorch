...
class Runner(nn.Module):
    def __init__(self, config_file, initial_state=None):
        ...
        self.optimizer = optim.Adam(self.initializer.parameters(), lr=1e-1)
    ...
    def execute(self):
        ...
        for episode in range(self.config['simulation_metadata']['num_episodes']):
            self.state = self.initializer.state #TODO: Check if this is sufficient for reset
            for traj_var in self.trajectory.keys():
                self.trajectory[traj_var].append(deque())

            for step in tqdm(range(self.config["simulation_metadata"]["num_steps_per_episode"])):
                ...
                for substep in self.config['substeps'].keys():
                    ...
                    #Learning after every sub-step
                    
                #Learning after every step
                
            #Learning after every episode        
