...
class UpdateMonthCounter(SubstepTransition):
    ...
    def forward(self, state, action):
        month_counter = get_by_path(state, re.split("/", self.input_variables['Month_Counter']))
        month_counter += 1
        if month_counter>12:
            month_counter = 1
        return {self.output_variables[0] : month_counter}
        
class UpdateSavings(SubstepTransition):
    def __init__(self,config, input_variables, output_variables, arguments):
        super().__init__(config, input_variables, output_variables, arguments)
    ...
    def calculateMonthlyIncome(self, state, action):
        ...
        # Initialise monthly income with 0
    ...
    def distributeTax(self, state, action, total_tax,zi):
        # Distribute tax
        num_agents = get_by_path(state, re.split("/", self.input_variables['Num_Agents']))
        total_tax = total_tax / num_agents
        zi = zi + total_tax
        return zi
    ...
    def increaseSavingsAnnualy(self, state, action):
