...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Benchmark standard vs. vectorized AgentTorch implementations')
    parser.add_argument('--std-config', type=str, default='config.yaml',
                        help='Path to standard config YAML (default: config.yaml)')
    parser.add_argument('--vmap-config', type=str, default='config_vmap.yaml',
    ...
    parser.add_argument('--predators', type=int, default=40,
                        help='Number of predators (default: 40)')
    parser.add_argument('--prey', type=int, default=80,
                        help='Number of prey (default: 80)')
...
def run_benchmark(runner_class, config, registry, num_runs=3, num_steps=10):
    ...
    for run in range(num_runs):
        ...
        print(f"Run {run+1}: initialization {init_time:.4f}s, simulation {run_time:.4f}s")
...
def run_scale_test(config_std, config_vmap, registry, args):
    ...
    for scale in scales:
        ...
        # Run benchmarks
        std_result = run_benchmark(Runner, config_std_copy, registry, 
                                   num_runs=1, num_steps=args.steps)
        vmap_result = run_benchmark(VectorizedRunner, config_vmap_copy, registry,
                                   num_runs=1, num_steps=args.steps)
...
def plot_scale_test_results(scale_results, output_dir="benchmark_results"):
    ...
    # Calculate speedups
    init_speedups = [s/v if v > 0 else 0 for s, v in zip(std_init_times, vmap_init_times)]
    sim_speedups = [s/v if v > 0 else 0 for s, v in zip(std_sim_times, vmap_sim_times)]
    step_speedups = [s/v if v > 0 else 0 for s, v in zip(std_step_times, vmap_step_times)]
    ...
    # Add overall title
    fig.suptitle('Performance Scaling: Standard vs. Vectorized Implementation', fontsize=16)
...
def main():
    ...
    # Otherwise, run standard benchmarks
        ...
        standard_results = run_benchmark(Runner, std_config, registry, args.runs, args.steps)
        
        # Run vectorized implementation benchmark
        vectorized_results = run_benchmark(VectorizedRunner, vmap_config, registry, args.runs, args.steps)
        ...
        # Plot results
        init_speedup, sim_speedup, step_speedup = plot_benchmark_results(results, output_dir)
        ...
        print(f"  Standard:   {standard_results['initialization']['mean']:.4f} ± {standard_results['initialization']['std']:.4f} seconds")
        print(f"  Vectorized: {vectorized_results['initialization']['mean']:.4f} ± {vectorized_results['initialization']['std']:.4f} seconds")
        print(f"  Speedup:    {init_speedup:.2f}x")
        
        print(f"\nSimulation:")
        print(f"  Standard:   {standard_results['simulation']['mean']:.4f} ± {standard_results['simulation']['std']:.4f} seconds")
        print(f"  Vectorized: {vectorized_results['simulation']['mean']:.4f} ± {vectorized_results['simulation']['std']:.4f} seconds")
        print(f"  Speedup:    {sim_speedup:.2f}x")
        
        print(f"\nAverage Step:")
        print(f"  Standard:   {standard_results['step_times']['mean']:.4f} ± {standard_results['step_times']['std']:.4f} seconds")
        print(f"  Vectorized: {vectorized_results['step_times']['mean']:.4f} ± {vectorized_results['step_times']['std']:.4f} seconds")
