...
class DecideMovementVmap(SubstepAction):
    ...
    def forward(self, state, observations):
        ...
        next_positions = []
        for i, (pos, e, neighbors, seed) in enumerate(zip(positions, energy, possible_neighbors, random_seeds)):
        ...
        next_positions = torch.stack([pos.squeeze() if pos.dim() > 1 else pos for pos in next_positions])
...
class UpdatePositionsVmap(SubstepTransition):
    ...
    def forward(self, state, action):
        ...
        if action is None or "prey" not in action or action["prey"] is None or "predator" not in action or action["predator"] is None:
