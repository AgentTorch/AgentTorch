...
class UpdateMacroRates(SubstepTransition):
    ...
    def __init__(self, config, input_variables, output_variables, arguments):
        ...
        self.labor_force_participation_rate_weight = torch.tensor(self.learnable_args['labor_force_participation_rate_weight'], requires_grad=True)
        
        brooklyn_claims_path = self.config['simulation_metadata']['brooklyn_claims_path']
        bronx_claims_path = self.config['simulation_metadata']['bronx_claims_path']
        manhattan_claims_path = self.config['simulation_metadata']['manhattan_claims_path']
        queens_claims_path = self.config['simulation_metadata']['queens_claims_path']
        staten_island_claims_path = self.config['simulation_metadata']['staten_island_claims_path']
        ...
        self.manhattan_claims = torch.tensor(manhattan_claims_df['beneficiaries'].values)
        self.queens_claims = torch.tensor(queens_claims_df['beneficiaries'].values)
        self.staten_island_claims = torch.tensor(staten_island_claims_df['beneficiaries'].values)
    ...
    def calculateUnemploymentRate(self, unemployment_adaptation_coefficient, labor_force, initial_claims):
        # unemployment_adaptation_coefficient = unemployment_adaptation_coefficient.reshape(
        current_total_unemployment_rate = unemployment_adaptation_coefficient[0]*torch.log(labor_force) + unemployment_adaptation_coefficient[1]*torch.log(initial_claims) + unemployment_adaptation_coefficient[2] #+ self.labor_force_participation_rate_weight*(labor_force_participation_rate)
        return current_total_unemployment_rate
    
    def calculateUnemploymentRateUsingNN(self,labor_force, initial_claims):
        output = self.model(torch.tensor([torch.log(labor_force), torch.log(initial_claims)]))
    ...
    def forward(self, state, action):
        ...
        labor_force = get_by_path(state, re.split("/", self.input_variables['labor_force']))
        ...
        unemployment_adaptation_coefficient_all = unemployment_adaptation_coefficient_all.reshape(5,3)
        ...
        labor_force_brooklyn = torch.clamp(torch.sum(working_status_county_brooklyn), min=1)
        labor_force_manhattan = torch.clamp(torch.sum(working_status_county_manhattan), min=1)
        labor_force_queens = torch.clamp(torch.sum(working_status_county_queens), min=1)
        labor_force_staten_island = torch.clamp(torch.sum(working_status_county_staten_island), min=1)
        ...
        current_unemployment_rate_county_bronx = self.calculateUnemploymentRate(unemployment_adaptation_coefficient_bronx, labor_force_bronx, current_month_initial_claims_bronx)
        current_unemployment_rate_county_brooklyn = self.calculateUnemploymentRate(unemployment_adaptation_coefficient_brooklyn, labor_force_brooklyn, current_month_initial_claims_brooklyn)
        current_unemployment_rate_county_manhattan = self.calculateUnemploymentRate(unemployment_adaptation_coefficient_manhattan, labor_force_manhattan, current_month_initial_claims_manhattan)
        current_unemployment_rate_county_queens = self.calculateUnemploymentRate(unemployment_adaptation_coefficient_queens, labor_force_queens, current_month_initial_claims_queens)
        current_unemployment_rate_county_staten_island = self.calculateUnemploymentRate(unemployment_adaptation_coefficient_staten_island, labor_force_staten_island, current_month_initial_claims_staten_island)
        current_nyc_unemployment_rate = (current_unemployment_rate_county_bronx + current_unemployment_rate_county_brooklyn + current_unemployment_rate_county_manhattan + current_unemployment_rate_county_queens + current_unemployment_rate_county_staten_island) / 5
        ...
        unemployment_rate = unemployment_rate + (current_nyc_unemployment_rate*time_step_one_hot)
