...
class Behavior():
    ...
    def sample(self, kwargs=None):
        ...
        sampled_behavior = torch.zeros(self.population.population_size, 1).to(kwargs['device']) 
        
        # Get list of prompts for each group
        prompt_list = self.prompt_manager.get_prompt_list(kwargs=kwargs)
        masks = self.get_masks_for_each_group(self.prompt_manager.dict_variables_with_values)
        ...
        sampled_behavior = self.get_sampled_behavior(sampled_behavior, masks, agent_output)
        
        # Save current step's conversation history to file
        # file_dir : Path to export current step's conversation history
        self.archetype.export_memory_to_file(file_dir = kwargs['current_memory_dir'],last_k = len(prompt_list)) 
    ...
    def get_masks_for_each_group(self, variables):
        masks = []
        for en, target_values in enumerate(self.prompt_manager.combinations_of_prompt_variables_with_index):
