...
race_mapping = {
    ...
    "white": ["WtNH"],
    "black": ["BlNH"],
    "native": ["NHPINH", "AIANNH"],
    "other": ["OthNH", "Rc2plNH"],
    "asian": ["AsnNH"],
}

age_mapping = {
    "U19": ["PopU5", "Pop5t9", "Pop10t14", "Pop15t19"],
    "20t29": ["Pop20t24", "Pop25t29"],
    "30t39": ["Pop30t34", "Pop35t39"],
    "40t49": ["Pop40t44", "Pop45t49"],
    "50t64": ["Pop50t54", "Pop55t59", "Pop60t64"],
    "65A": ["Pop65t69", "Pop70t74", "Pop75t79", "Pop80t84", "Pop85pl"],
}

education_mapping = {
    "high_school": ["EA_LT9G", "EA_9t12ND", "EA_HScGrd"],
    "college_degree": ["EA_SClgND", "EA_AscD", "EA_BchD"],
    "graduate_degree": ["EA_GrdPfD"],
    "studying": ["Pop3plEn"],
}

employment_insurance_mapping = {
    "employed_insured": "EmHIns",
    "employed_uninsured": "EmNHIns",
    "unemployed_insured": "UEmHIns",
    "unemployed_uninsured": "UEmNHIns",
    "nolabor_insured": "NLFHIns",
    "nolabor_uninsured": "NLFNHIns",
}
...
def get_gender_split(total_estimate, total_percentage, male_ratio):
    ...
    male_percentage = int(max(0, male_ratio * total_percentage.item()))
    ...
    return_dict = {
        "male": [male_estimate, male_probability],
        "female": [female_estimate, female_probability],
    }
...
def get_nta_age_gender(df, nta_id, age_mapping, male_ratio=0.508):
    ...
    for key in age_mapping:
        ...
        if key == "U19":
            male_ratio = (
                compute_stats(nta_df, "PopU18M")[-1] / 100.0
            )  # percentage of male < 19
        if key == "65A":
            male_ratio = (
                compute_stats(nta_df, "Pop65plM")[-1] / 100.0
            )  # percentage of male > 65

        gender_split_stats = get_gender_split(
            total_estimate, total_percentage, male_ratio=male_ratio
        )
        ...
        nta_age_gender[key_male] = {
            "estimate": male_stats[0],
            "probability": male_stats[1],
        }
        nta_age_gender[key_female] = {
            "estimate": female_stats[0],
            "probability": female_stats[1],
        }
...
def get_nta_education(df, nta_id, education_mapping):
    ...
    for key in education_mapping:  # education_mapping is for >25 yr old agents
        ...
        nta_education[key]["probability"] = (
            nta_education[key]["estimate"] / total_educated_studying
        )
