...
def process_nta_household(NTA_ID):
    ...
    nta_household_ownership = get_nta_household_ownership(
        house_df, NTA_ID, ownership_mapping
    )

    num_households_income = sum(
        [nta_household_income[key_ix]["estimate"] for key_ix in nta_household_income]
    )
    num_households_onwership = sum(
        [
            nta_household_ownership[key_ix]["estimate"]
            for key_ix in nta_household_ownership
        ]
    )
    ...
    income_prob = [
        nta_household_income[key_ix]["probability"] for key_ix in nta_household_income
    ]
    ownership_prob = [
        nta_household_ownership[key_ix]["probability"]
        for key_ix in nta_household_ownership
    ]
